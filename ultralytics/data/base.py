...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            paired_mask (bool): If True, read an additional mask image and concatenate with RGB as extra channels.
            mask_dir (str): Directory name used to locate paired masks by replacing `/images/` in image paths.
        """
        super().__init__()
    ...
    def _resolve_mask_path(self, image_path: str) -> str:
        """Resolve paired mask path from image path by replacing /images/ with configured mask directory."""
        marker = f"{os.sep}images{os.sep}"
        replacement = f"{os.sep}{self.mask_dir}{os.sep}"
        if marker in image_path:
            return image_path.replace(marker, replacement, 1)
        return image_path.replace("images", self.mask_dir, 1)
    ...
    def load_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    im = np.load(fn)
                    if self.paired_mask and (im.ndim != 3 or im.shape[2] != self.channels):
                        im = None
                except Exception as e:
                ...
                im = imread(f, flags=self.cv2_flag)  # BGR
            if im is None:
                im = imread(f, flags=self.cv2_flag)  # BGR fallback when cached npy channel count mismatches
            if im is None:
            ...
            mask = None
            if self.paired_mask:
                mask_path = self._resolve_mask_path(f)
                mask = imread(mask_path, flags=cv2.IMREAD_COLOR)
                if mask is None:
                    raise FileNotFoundError(f"Mask Not Found {mask_path} for image {f}")
    ...
    def cache_images_to_disk(self, i: int) -> None:
        ...
        if not f.exists():
            ...
            if self.paired_mask:
                mask = imread(self._resolve_mask_path(self.im_files[i]), flags=cv2.IMREAD_COLOR)
                if im is not None and mask is not None:
                    if mask.shape[:2] != im.shape[:2]:
                        mask = cv2.resize(mask, (im.shape[1], im.shape[0]), interpolation=cv2.INTER_LINEAR)
                    im = np.concatenate((im, mask), axis=2)
            if im is not None:
